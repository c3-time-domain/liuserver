...
if [ ! -f $POSTGRES_DATA_DIR/PG_VERSION ]; then
    ...
    if [[ $PGUSER != "postgres" ]]; then
        ...
        psql --comand "CREATE USER $PGUSER PASSWORD '$PGPASS' CREATEDB"
    fi
    echo "Creating database $PGNAME"
    psql --command "CREATE DATABASE $PGNAME OWNER $PGUSER"
    psql --command "CREATE EXTENSION q3c" $PGNAME
